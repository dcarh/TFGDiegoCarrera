...
import modelClasses.app.media.{Movie, TVShow, Season, Episode, MovieId, TVShowId}
...
object GetRequestExample extends IOApp {
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    tmdbClient.generalRequestNotGeneralized(Movies.requestedSimilarMoviesEndpoint, MovieId(550)).flatMap {
      case Right(resource) =>
